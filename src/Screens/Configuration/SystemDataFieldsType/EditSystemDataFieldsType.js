...
function NewSystemDataFieldsType() {
  const [name, setName] = useState("Data Field Name 01");
  const [dataFieldType, setDataFieldType] = useState(2);
  const [selectPickList, setSelectPickList] = useState("");
  const [pickListName, setPickListName] = useState("");
  const [defineNo, setDefineNo] = useState(45);
